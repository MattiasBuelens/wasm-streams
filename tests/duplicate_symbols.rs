...
//!
//! The actual check lives in the `duplicate-symbols` crate at
//! `tests/duplicate-symbols`; building it for `wasm32-unknown-unknown` *is*
//! the test; a duplicate symbol turns into a `rust-lld` link error. This
//! just drives that build from `cargo test`, so it runs on CI without a
//! dedicated job.
//!
...
fn duplicate_symbols() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/duplicate-symbols");
